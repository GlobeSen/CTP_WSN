...
bool time_in_period(period nowtime,period range)
{
	if(nowtime.start_time>=range.start_time && nowtime.end_time<=range.end_time)
		return TRUE;
	else
		return FALSE;
}

bool time_equal(period t1,period t2)
{
