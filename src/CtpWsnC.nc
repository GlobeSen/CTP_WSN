...
//	uses interface Receive as DataRec;
	...
	uses interface Timer<TMilli> as TimeUpdate;
...
//	Msg * dataReceive;//DataRec接收数据
	...
	message_t packet; 
	message_t time_packet;
	message_t data_packet;//查询节点传感器返回数据
//	bool busy=FALSE;
	
	uint8_t fibCount;
	uint8_t fibCount_Time;
...
//		printfflush();
		...
		for(i = 0; i < pitNum; i++) {
			if((ability_equal((receive->msgName).ability, (pitTable[i].pitName)
					...
					.ability)) && time_in_period((receive->msgName).time,(pitTable[i].pitName).time)) {
...
//					pitNum--;
	...
	event void AMControl.startDone(error_t error) {
		...
		if(error != SUCCESS) {
			...
			call TimeUpdate.startPeriodic(1000);
		}
	}
...
//		printfflush();
	...
	//		currentType = humidityType;
			...
			csTable[0].csName.time.start_time=csTable[0].csName.time.end_time=current_time;
			tmp = (Msg * )(call Packet.getPayload(&data_packet , sizeof(Msg)));
			tmp->msgType = DATA;
			memcpy(&(tmp->msgName), &(csTable[0].csName), sizeof(name));
			tmp->data = csTable[0].data;
	//		tmp->hop=receive->hop;
			if(call AMSend.send(addr, &data_packet, sizeof(Msg)) == SUCCESS)
			{
				printf("find in repo ,return to %d\n",addr);
				printfflush();
			}
		}
	}
	...
	//		currentType = tempType;
			...
			csTable[1].csName.time.start_time=csTable[1].csName.time.end_time=current_time;
			tmp = (Msg * )(call Packet.getPayload(&data_packet , sizeof(Msg)));
			tmp->msgType = DATA;
			memcpy(&(tmp->msgName), &(csTable[1].csName), sizeof(name));
			tmp->data = csTable[1].data;
//			tmp->hop=receive->hop;
			if(call AMSend.send(addr, &data_packet, sizeof(Msg)) == SUCCESS)
			{
				printf("find in repo ,return to %d\n",addr);
	...
	//		currentType = lightType;
			...
			csTable[2].csName.time.start_time=csTable[2].csName.time.end_time=current_time;
			tmp = (Msg *)(call Packet.getPayload(&data_packet , sizeof(Msg)));
			tmp->msgType = DATA;
			memcpy(&(tmp->msgName), &(csTable[2].csName), sizeof(name));
			tmp->data = csTable[2].data;
//			tmp->hop=receive->hop;
			if(call AMSend.send(addr, &data_packet, sizeof(Msg)) == SUCCESS)
			{
				printf("find in repo ,return to %d\n",addr);
...
//				printfflush();
	...
	task void Time_syc()
	{
			while(fibCount_Time<fibNum)
			{
				Msg* tmp= (Msg * )(call Packet.getPayload(&time_packet, sizeof(Msg)));
				memset(tmp, 0, sizeof(TimeMsg));
				
				tmp->msgType = TIME;
				tmp->msgName.time.start_time=tmp->msgName.time.end_time=current_time;
				if(call FibSend.send(fibTable[fibCount_Time].goId, &time_packet, sizeof(Msg)) == SUCCESS)
				{
					printf("send time to %d\n",fibTable[fibCount_Time].goId);
					printfflush();
					fibCount_Time++;
					return;
...
//		dbg("MsgRec","sizeof(Msg) = %d\n",sizeof(Msg));
		if(len==sizeof(TimeMsg))
		{	
			uint16_t* tmp=(uint16_t*)payload;
			current_time=*tmp;
			fibCount_Time=0;
			post Time_syc();
			return msg;	
		...
		if(len == sizeof(Msg)) {
			...
			if(receive->msgType == IN) {
				location a=(receive->msgName).ability;
				period IN_time_range=(receive->msgName).time;
				#ifndef DEBUG
				printf("receive a Interest Packet!\n");
				printf("The IN's range:/(%d,%d)/(%d,%d)\n", a.leftUp.x, a.leftUp.y, a
				.rightDown.x, a.rightDown.y);
				printf("The IN's Time: (%u,%u)",IN_time_range.start_time,IN_time_range.end_time);
...
//				printfflush();
				...
				if(cs_query_In(receive, msg)) {
					...
					if(current_time>=IN_time_range.start_time && current_time<=IN_time_range.end_time 
					&& location_belong(thisNode.nodeLocation,a))
					{
							printf("sensor read \n");
							printfflush();		
							if((receive->msgName).dataType == lightType) {
								call LightRead.read();
							}
							if((receive->msgName).dataType == humidityType) {
								call HumidityRead.read();
							}
							if((receive->msgName).dataType == tempType) {
								call TempRead.read();
							}
					}
					for(i = 0; i < fibNum; i++) {
...
//				printf("data:%d\n",receive->data);
			...
			if(receive->msgType == TIME)
			{
				
				current_time=receive->msgName.time.start_time;
				fibCount_Time=0;
				post Time_syc();
				return msg;	
			}
	...
	bool cs_query_In(Msg * Rec, message_t * rec) {
		...
		for(i = 0; i < csNum; i++) {
			...
			&& time_in_period((csTable[i].csName).time,(Rec->msgName).time)) {
...
//						
	...
	bool pit_query_In(Msg * Rec, message_t * rec) {
		...
		for(i = 0; i < pitNum; i++) {
			...
			&& time_equal((Rec->msgName).time,(pitTable[i].pitName).time)) {
				type0 = (Rec->msgName).dataType;
				type1 = pitTable[i].pitName.dataType;
				if(type0 == type1) {
					pitTable[i].weight++;
...
//			printfflush();
	...
	bool cs_query_Data(Msg * ndwRec, message_t * rec) {
		...
		for(i = 0; i < csNum; i++) {//if in cs then return
			...
			&& time_equal((ndwRec->msgName).time,(csTable[i].csName).time)) {
...
//					printfflush();
		...
		if(csNum >= CSMAX) {//cache replacement algorithm:popularity and content value
			...
			for(i=0;i<cacheMapSize;i++){//find item whether in the cacheMap
				...
				&& time_equal((csTable[maxTag].csName).time, (cacheMap[i].msgName).time)){ 
	...
	event message_t * InfoRec.receive(message_t * msg, void * payload,
		...
		if(TOS_NODE_ID != 1) {
			...
			if((call InfoSend.send(0, msg, sizeof(nodeInfo))) == SUCCESS) {
				#ifndef DEBUG
				printf("the node location is (%d,%d)\n",tmp->nodeLocation.leftUp,tmp->nodeLocation.rightDown);
